...
- **Hub — zero-setup student devices (v1.3.1):** the app is client-side, so every device starts empty. `ROSTER_SEED` (edit-per-deployment constant next to `SUBJECTS`) bakes the class roster into the app — a fresh device shows name tiles immediately; students NEVER see teacher setup. `TEACHER_PASS_SEED` optionally pre-sets the teacher passcode everywhere. Seeds merge when the seed list changes (`g7.seedv`) without touching PINs, progress, or teacher-added names. **Whenever the roster changes, update `ROSTER_SEED` in the file and redeploy** — Manage roster only edits one device.
